...
if [ "${LOOPBACK_ENABLED}" == "true" ]; then
    sed -i -r 's/ #(allow)/ \1/' $NGINX_CONF_PATH/ds.conf.tmpl $NGINX_CONF_PATH/ds-ssl.conf.tmpl
    sed -i -r 's/ #(deny)/ \1/' $NGINX_CONF_PATH/ds.conf.tmpl $NGINX_CONF_PATH/ds-ssl.conf.tmpl
else
    sed -i -r 's/ (allow)/ #\1/' $NGINX_CONF_PATH/ds.conf.tmpl $NGINX_CONF_PATH/ds-ssl.conf.tmpl
    sed -i -r 's/ (deny)/ #\1/' $NGINX_CONF_PATH/ds.conf.tmpl $NGINX_CONF_PATH/ds-ssl.conf.tmpl
fi
